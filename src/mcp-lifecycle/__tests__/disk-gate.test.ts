...
describe("atomicWriteFile", () => {
  it("replaces the destination with a complete file", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-atomic-"));
...
describe("acquireProcessLock", () => {
  ...
  it("throws ProcessLockBusyError when another live pid holds the lock", async () => {
    lockDir = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-lock-"));
    lockDir = path.join(lockDir, ".write.lock");
  ...
  it("steals a stale pid", async () => {
    lockDir = await fs.mkdtemp(path.join(os.tmpdir(), "mcp-stale-"));
    lockDir = path.join(lockDir, ".write.lock");
