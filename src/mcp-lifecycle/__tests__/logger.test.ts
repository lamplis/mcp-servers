...
describe("createFileLogger", () => {
  ...
  it("writes ts with a local offset matching the file date", () => {
    logDir = makeTempDir();
    const now = new Date(2026, 8, 10, 0, 5, 29, 123);
    const logger = createFileLogger({
      logDir,
      now: () => now,
      stderr: new MemoryStderr(),
    });
    logger.info("offset.check");
    const lines = readLogLines(path.join(logDir, "2026-09-10.log"));
    expect(lines[0].ts).toBe(formatLocalIsoOffset(now));
    expect(String(lines[0].ts)).toMatch(/^2026-09-10T00:05:29\.123[+-]\d{2}:\d{2}$/);
  });
