...
export async function readLockHolderPid(
  lockDir: string
...
export async function removeLockDir(lockDir: string): Promise<void> {
...
export async function acquireProcessLock(
  ...
  for (let attempt = 0; attempt <= retries; attempt += 1) {
    try {
      ...
      const stale =
        holderPid === undefined || (holderPid !== pid && !alive(holderPid));
      if (stale) {
        await removeLockDir(lockDir);
        continue;
      }
