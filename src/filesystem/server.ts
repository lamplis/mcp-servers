...
export type FilesystemServerFactoryOptions = {
  initialAllowedDirectories?: string[];
  validateInitialDirectories?: boolean;
  allowEmptyDirectories?: boolean;
};

export type FilesystemServerFactoryResponse = {
  server: McpServer;
  cleanup: (sessionId?: string) => void;
};

async function resolveAllowedDirectories(
  directories: string[] = [],
...
export async function createServer(
  options: FilesystemServerFactoryOptions = {}
): Promise<FilesystemServerFactoryResponse> {
  const allowEmpty = options.allowEmptyDirectories ?? false;
  const initialDirectories = options.initialAllowedDirectories ?? [];
  if (!allowEmpty && initialDirectories.length === 0) {
  ...
  let allowedDirectories = await resolveAllowedDirectories(
    initialDirectories,
    options.validateInitialDirectories ?? true
  );
  if (!allowEmpty && allowedDirectories.length === 0) {
    console.warn("Starting without allowed directories; waiting for client roots.");
  }
...
server.server.oninitialized = async () => {
  ...
  if (clientCapabilities?.roots) {
    ...
    console.error("Server started without allowed directories and client does not support MCP roots protocol.");
    console.error("Please provide directories via command-line arguments or use a client that supplies roots.");
  }
};
