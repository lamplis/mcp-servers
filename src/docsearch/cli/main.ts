...
program
  ...
  .action(async (source: string, options: { watch?: boolean }, cmd: Command) => {
    ...
    try {
      ...
      // Validate source
      if (!['file', 'files', 'confluence', 'url', 'urls', 'all'].includes(source)) {
        console.error(`Invalid source: ${source}. Must be one of: files, urls, confluence, all`);
        process.exit(1);
      }
  ...
  .action(
    async (
      ...
      try {
        ...
        if (options.source && !['file', 'confluence', 'url'].includes(options.source)) {
          console.error('Invalid source. Must be file, url, or confluence.');
          process.exit(1);
        }
        ...
        const searchCommand: SearchCommand = {
          query,
          topK,
          source: options.source as 'file' | 'confluence' | 'url' | undefined,
          repo: options.repo,
