...
### 3. Placeholders (template copy only)
...
Reload the window to activate the MCP servers.

### What You Get

The rule file (`mcp-servers.md`) teaches the AI to:
...
2. Run `python scripts/setup_roo.py` (or copy `mcp-config-template.json` to `.roo/mcp.json` / `.cursor/mcp.json`)
...
This fork does not ship or maintain a third-party MCP catalog. Public SaaS servers from the old community list are not part of this runtime and typically cannot be used here (GitHub-only egress, internal npm registry, no Docker).

Browse published servers on [the MCP Registry](https://registry.modelcontextprotocol.io/). The supported stack in this repository is the local JSON MCP set: `npx` TypeScript servers plus optional Python examples, with JSON/JSONL persistence and no SQLite.
