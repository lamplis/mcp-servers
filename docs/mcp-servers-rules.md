...
| Symptom | Action |
|---------|--------|
| MCP tool missing / timeout | First `node scripts/mcp-ps.mjs doctor`. Then reload + `python scripts/setup_roo.py --check` and `node scripts/validate_mcps.mjs`. Continue with IDE tools only for the **current workspace**. |
